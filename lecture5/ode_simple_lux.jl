...
# ╔═╡ e98d8be0-b2ca-11ed-18f3-f91bdd67e1c6
using OrdinaryDiffEq, Plots, PlutoUI

# ╔═╡ 9621a9d2-3288-495f-a994-6df05b4f2dbe
using Lux, Random, Optimisers, Zygote

# ╔═╡ 8fd8fafb-95b6-45dc-b7c9-f0d005af808b
md"
Initial Condition
"

# ╔═╡ 431ca280-227e-4b7e-bab5-f9ced0133e79
@bind u0  Slider(0:0.1:2; default=1, show_value=true)

# ╔═╡ 6c233b75-8a35-451c-9b46-827776366cb6
md"
Parameter
"

# ╔═╡ ef6cc010-6fa4-4e57-a116-e05fc9db04df
@bind p Slider(-2:0.1:2; default=1, show_value=true)

# ╔═╡ 0a595460-fc29-497d-b349-021afef6b931
f(u, p, t) = 10 * sin(t * 30) + p * u

# ╔═╡ 3e1c3548-7011-4da2-a393-1c97bbddb1b9
begin	
  tspan = (0.0, 1.0)
...
# ╔═╡ c8e086d6-31b3-4728-8f04-e28aa6898cf9
begin
	plot(sol; linewidth=5, title="Solution to u'=f(u,t)",
	     xaxis="Time (t)", yaxis="u(t)", label="solution", ylims=(0,3))
end

# ╔═╡ 60c60c51-e5da-44d6-a4e9-4cb24d8ba325
md"
A Simple scalar Neural Network
"

# ╔═╡ 33623429-a0ad-4a7f-af09-3b386a2a12aa
n = 3

# ╔═╡ 76adb95c-dede-4553-a740-8d23c854f729
begin
  W1 = rand(n)
  W2 = rand(1, n)
...
# ╔═╡ acdebf53-07a2-402f-a49d-5d7ab6d612f1
simple(1)

# ╔═╡ ccf8513a-62b8-432d-a32c-9c72940ab551
NNODE = Chain(; hlayer_1=Dense(1, n, tanh), hlayer_2=Dense(n, 1))

# ╔═╡ d7cb5403-e4ca-4492-bbff-716a3f1e4cc4
begin
  rng = Xoshiro(1234)
  ps, st = Lux.setup(rng, NNODE)  # initialize the model
end

# ╔═╡ f81e4fb1-44ed-46f1-baa8-98a2b6aa790f
ps.hlayer_1.weight

# ╔═╡ 85759f3a-fdc1-4ce5-af80-29441422db4d
ps.hlayer_2.weight

# ╔═╡ 19cb16c1-250e-4dec-947e-7caec9998e84
md"
Let's solve u' = cos(2πt), u(0) = 1
"

# ╔═╡ 84c31418-b091-4e46-ae9e-8ae2923746dc
function g(model, t, ps, st)
  y, st_ = model(t, ps, st)
  return t .* y .+ 1.0f0, st_
end

# ╔═╡ e1132d48-8131-454d-ac8f-2f4ec656b044
function g_diff(model, t, ps, st, ϵ)
  y1, st_ = g(model, t .+ ϵ, ps, st)
  y2, st_ = g(model, t, ps, st_)
  return (y1 .- y2) ./ ϵ, st_
end

# ╔═╡ 7722eacb-9a7f-4a85-bfaf-30d4b04d8512
begin
  using Statistics # for the loss
  ϵ = sqrt(eps(Float32))

  function loss(model, ps, st)
	t = reshape(collect(0.0f0:1.0f-2:1.0f0), 1, :)
	y, st_ = g_diff(model, t, ps, st, ϵ)
    return mean(abs2, y .- cos.(2π .* t)), st_
  end
end

# ╔═╡ f48b5a10-97c8-4c92-854b-f7116e06dc46
loss(NNODE, ps, st)

# ╔═╡ a073f4d4-bae3-4709-9b08-36bb3fda2dcb
begin
  opt = Adam(0.1f0)
  st_opt = Optimisers.setup(opt, ps)
end

# ╔═╡ a833ac57-2700-4b53-8303-48bbc543b66e
function train_model(loss_function, model, ps, st, st_opt;
					 total_iterations=10000)
  # Lux doesn't like global stuff, so let's define functions
  for iter in 1:total_iterations
    (loss, st), back = Zygote.pullback(p -> loss_function(model, p, st), ps)
...
# ╔═╡ 209cf4b5-9bbf-431d-895f-2755c8735da9
ps_trained, st_final, _ = train_model(loss, NNODE, ps, st, st_opt)

# ╔═╡ a6d1927e-87af-41aa-b546-a3a191b772b8
begin
  t_plot = 0:0.01:1.0
  plot(t_plot, vec(g(NNODE, reshape(t_plot, 1, :), ps_trained, st_final)[1]), label="NN")
  plot!(t_plot, 1.0 .+ sin.(2π .* t_plot) / 2π, label = "True Solution")
end
...
manifest_format = "2.0"
project_hash = "2809f85cfd5be90151a244577348413727a6468d"
...
[[deps.Lux]]
deps = ["Adapt", "CUDA", "ChainRulesCore", "Functors", "LinearAlgebra", "LuxCore", "LuxLib", "Markdown", "NNlib", "Optimisers", "Random", "Requires", "Setfield", "SparseArrays", "Statistics", "cuDNN"]
git-tree-sha1 = "d41d84224efbcedc852bef0918df707f4804c132"
...
git-tree-sha1 = "c82f038c56d9c50be0de6ad5acd6cee016bd66dd"
